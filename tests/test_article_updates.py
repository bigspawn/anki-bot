...
class TestArticleUpdates:
    ...
    def temp_db_manager(self):
        ...
        test_words = [
            ...
            },
            {
                ...
                "example": "Meine Geschwister sind älter.",
            },
            {
                "lemma": "Großeltern",
                "part_of_speech": "noun",
                "article": None,
                "translation": "дедушка и бабушка",
                "example": "Meine Großeltern besuchen uns.",
            },
            {
                "lemma": "Pommes",
                "part_of_speech": "noun",
    ...
    def test_plural_nouns_without_articles(self, temp_db_manager):
        ...
        for word in words:
            if word["lemma"] in plural_words:
                ...
                assert word["article"] is None, (
                    f"{word['lemma']} должно иметь article = None"
                )

                # Проверяем форматирование в utils.py
                formatted = format_word_display(word)
                assert "None" not in formatted, (
                    f"Форматирование не должно содержать 'None': {formatted}"
                )
                assert f"🇩🇪 {word['lemma']}" in formatted, (
                    f"Должно содержать только имя слова: {formatted}"
                )
                ...
                if article and article != "None" and article.strip():
                    word_display = (
                        f"{article} {word['lemma']} - {word['part_of_speech']}"
                    )
                else:
                    word_display = f"{word['lemma']} - {word['part_of_speech']}"

                assert word_display == f"{word['lemma']} - noun", (
                    f"Session manager должен показывать без артикля: {word_display}"
                )
                assert "None" not in word_display, (
                    f"Session manager не должен показывать 'None': {word_display}"
                )
    ...
    def test_singular_nouns_with_articles(self, temp_db_manager):
        ...
        for word in words:
            if word["lemma"] in expected_articles:
                ...
                assert word["article"] == expected_article, (
                    f"{word['lemma']} должно иметь артикль {expected_article}, но имеет {word['article']}"
                )

                # Проверяем форматирование в utils.py
                formatted = format_word_display(word)
                assert f"🇩🇪 {expected_article} {word['lemma']}" in formatted, (
                    f"Должно содержать артикль: {formatted}"
                )
                assert "None" not in formatted, (
                    f"Не должно содержать 'None': {formatted}"
                )
                ...
                assert word_display == expected_display, (
                    f"Session manager должен показывать с артиклем: {word_display}"
                )
    ...
    def test_database_consistency_after_updates(self, temp_db_manager):
        ...
        for lemma, article in nouns:
            assert article != "None", (
                f"Слово {lemma} не должно иметь артикль 'None' (строка)"
            )

            # Если артикль есть, то он должен быть правильным
            if article is not None:
                assert article in ["der", "die", "das"], (
                    f"Слово {lemma} имеет неправильный артикль: {article}"
                )
    ...
    def test_word_display_formatting_consistency(self, temp_db_manager):
        ...
        for word in words:
            ...
            assert "None" not in formatted_utils, (
                f"Utils formatting содержит 'None': {formatted_utils}"
            )
            assert "None" not in session_display, (
                f"Session manager formatting содержит 'None': {session_display}"
            )

            # Если есть артикль, оба должны его показывать
            if word.get("article"):
                assert word["article"] in formatted_utils, (
                    f"Utils должен показывать артикль {word['article']}: {formatted_utils}"
                )
                assert word["article"] in session_display, (
                    f"Session manager должен показывать артикль {word['article']}: {session_display}"
                )
    ...
    def test_specific_corrected_words(self, temp_db_manager):
        ...
        for word in words:
            if word["lemma"] in corrected_words:
                ...
                formatted = format_word_display(word)
                assert "None" not in formatted, (
                    f"Исправленное слово {word['lemma']} не должно показывать 'None': {formatted}"
                )
        ...
        edge_cases = [
            {
                "lemma": "TestEmpty",
                "article": "",
                "expected_display": "TestEmpty - noun",
            },
            {
                "lemma": "TestNull",
                "article": None,
                "expected_display": "TestNull - noun",
            },
            {
                "lemma": "TestSpace",
                "article": "   ",
                "expected_display": "TestSpace - noun",
            },
        ]
        ...
        for word in words:
            for case in edge_cases:
                if word["lemma"] == case["lemma"]:
                    ...
                    assert word_display == case["expected_display"], (
                        f"Граничный случай {case['lemma']}: ожидалось {case['expected_display']}, получено {word_display}"
                    )
                    assert "None" not in word_display, (
                        f"Граничный случай не должен содержать 'None': {word_display}"
                    )
    ...
    def test_real_world_scenario_multiple_users(self, temp_db_manager):
        ...
        for user_id in [first_user_id, second_user_id]:
            ...
            for word in words:
                ...
                formatted = format_word_display(word)
                assert "None" not in formatted, (
                    f"Пользователь {user_id}, слово {word['lemma']}: форматирование содержит 'None'"
                )
                ...
                assert "None" not in word_display, (
                    f"Пользователь {user_id}, слово {word['lemma']}: session display содержит 'None'"
                )
